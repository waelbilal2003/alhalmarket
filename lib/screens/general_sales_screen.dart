...
class GeneralSalesScreen extends StatefulWidget {
  const GeneralSalesScreen({super.key});

  @override
  State<GeneralSalesScreen> createState() => _GeneralSalesScreenState();
}

class _GeneralSalesScreenState extends State<GeneralSalesScreen> {
  ...
  Future<void> _loadInitialData() async {
    ...
    if (_employees.isNotEmpty) {
      _employeeName = _employees.first['name'];
    }

    // الحصول على رقم اليومية التالي
                ...
                children: [
                  ...
                  TextFormField(
                    decoration: const InputDecoration(
                        labelText: 'الهوية (الوزن الإجمالي)'),
                    keyboardType: TextInputType.number,
                    onChanged: (value) =>
    ...
    try {
      ...
      if (existingDaily == null) {
        dailyId = await _db.insertDaily({
          ...
          'date': _currentDate,
          'employee_id': 1, // يمكن تحديثه حسب الموظف المختار
        ...
        // تحديث المجموع الكلي
        await _db.updateInvoice(dailyId, {
          'total_cash': (existingDaily['total_cash'] as num) + _totalCash,
        });
      }

      // إنشاء الفاتورة
      final invoiceNumber =
          await _db.getNextInvoiceNumber(_currentDate, 'عامة');
  ...
  Widget build(BuildContext context) {
    return Directionality(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: const Text('صفحة المبيعات العامة'),
                  ...
                  ? Center(
                      child: Column(
                        ...
                        children: [
                          ...
                          const SizedBox(height: 8),
                          Text(
                            'اضغط على زر "إضافة" لإضافة عناصر',
                            style: TextStyle(
                              fontSize: 14,
